...
import { verifyAdminLoginChallenge } from "./login-challenge.js";

export const login: AppRouteHandler<LoginRoute> = async (c) => {
    const { loginName, password } = await c.req.json()
    const challenge = await verifyAdminLoginChallenge(c, { loginName })

    if (!challenge.ok) {
        return R.err(c, "登录校验失败", 401)
    }
...
export const logout: AppRouteHandler<LogoutRoute> = async (c) => {
    ...
    if (verified.payload.aud !== 'admin') return R.err(c, "token 类型不匹配", 401)
